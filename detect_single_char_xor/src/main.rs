...
fn main() {
	 	let f = File::open("encrypted_file.txt").expect("Unable to open file");
    ...
    for line in f.lines() {
        let line = line.expect("Unable to read line");
       	let decrypted_value = decrypt_string(line);
       	let score = calc_english_score(&decrypted_value);
       	if score > high_score {
       		high_score = score;
       		high_value = decrypted_value;
       	}
    }
   	println!("{}", high_value) 
}
